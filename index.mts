...
let exitCallback: ExitHandler = () => {};
function onExit(fn: ExitHandler) {
  exitCallback = fn;
}
  ...
  if (newPin >= 100 && newPin <= 999) {
    ...
    console.log(
      chalk.greenBright(
        "Your PIN:",
        pin,
        "has been set successfully to authenticate your account!"
      )
    );
  } else {
  ...
  switch (choice.action) {
    case "DepositAmount":
      ...
      if (depositAmount >= 500) {
        ...
        console.log(
          chalk.greenBright("Amount deposited successfully: ", depositAmount)
        );
    ...
    case "WithdrawAmount":
      if (balance >= 100) {
        ...
        if (WithdrawAmount <= balance) {
          ...
          console.log(
            chalk.greenBright("Amount withdrawn successfully: ", WithdrawAmount)
          );
          console.log(chalk.magentaBright("Your total balance is: ", balance));
          await choicer();
        } else {
        ...
        console.log(
          chalk.redBright(
            "You have insufficient balance. The minimum balance required is 100."
          )
        );
        await choicer();
      }
    ...
    case "ManagePIN":
      ...
      switch (confirmation.Guard) {
          ...
          if (pin) {
            ...
            if (PreviousPin === pin) {
              ...
              if (NewPin1 >= 100 && NewPin1 <= 999) {
                ...
                console.log(
                  chalk.redBright("Please enter a PIN between 100 and 999!")
                );
                await choicer();
                return;
              }
        ...
        case "ForgetPin":
          ...
          if (
            Guard3.ForgetPin.length > 12 && Guard3.ForgetPin.length < 14
          ) {
      ...
      break;
    case "Exit":
        console.log(chalk.blueBright("Thank you for using our ATM service"));
        ...
        try {
          await Promise.resolve(exitCallback());
        } catch (err) {
          console.error(chalk.redBright("Error in exit callback:"), err);
        }
        return;
        break;
    default:
...
let plusBalance = (amount: number) => balance += amount;
let minusBalance = (amount: number) => balance -= amount;
function getBalance(): number {
  return balance;
}
// ATM();
export {ATM,getBalance,plusBalance,minusBalance,NIC,onExit};
